	...
	If ($mode="t")
		...
		If ($sharedStrings.exists)
			...
			If (OK=1)
				...
				$si:=DOM Find XML element:C864($sst;"sst/si";$sis)
				If (OK=1)
					For ($i;1;Size of array:C274($sis))
						
						$si:=$sis{$i}
						ARRAY TEXT:C222($ts;0)
						  //$t:=DOM Find XML element($si;"t";$ts) //v19以降で標準Xpathを有効にしている場合
						$t:=DOM Find XML element:C864($si;"si/t";$ts)
						If (OK=1)
											...
											If (OB Is defined:C1231($values;$cellRef))
												...
												Case of 
													: ($mode="f")
														  ...
														  //$f:=DOM Find XML element($c;"f")//v19以降で標準Xpathを有効にしている場合
														$f:=DOM Find XML element:C864($c;"c/f")
														If (OK=0)
														...
														DOM SET XML ELEMENT VALUE:C868($f;$values[$cellRef].f)
														  ...
														  //$v:=DOM Find XML element($c;"v")//v19以降で標準Xpathを有効にしている場合
														$v:=DOM Find XML element:C864($c;"c/v")
